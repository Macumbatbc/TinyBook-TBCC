...
function TSB_SpellRankButton_OnEnter(self)
    ...
    if (GameTooltip:SetSpellBookItem(spellId, bookType)) then -- NOTE: Returns nil if spell doesn't exist.
...
function TSB_SpellRankButton_PostClick(self, button)
    ...
    if (IsModifiedClick("CHATLINK")) then
        if (MacroFrame and MacroFrame:IsShown()) then
            local spellName, subSpellName = GetSpellBookItemName(spellId, bookType); -- NOTE: Returns nil if spell doesn't exist.
    ...
    if (IsModifiedClick("PICKUPACTION")) then
        PickupSpellBookItem(spellId, bookType); -- NOTE: Returns nil (and does nothing except clear cursor contents) if spell doesn't exist.
        return;
    end
end
