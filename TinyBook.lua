...
local IsSelectedSpell = IsSelectedSpellBookItem;
    ...
    hooksecurefunc("ToggleSpellBook", function(bookType)
        ...
        if (allowNextBlizzardSpellBook or TSB_CombatLockdown.inCombat or (not isBlizzardShown)) then -- Use Blizzard's spellbook/API handler.
            ...
            if (isBlizzardShown) then
                PlaySound(SpellBookFrame.bookType == BOOKTYPE_SPELL and 829 or 834);
            end
...
function TSB_SpellBookFrame_OnEvent(self, event, ...)
    ...
    if (event == "PLAYER_LOGIN" or event == "ADDON_LOADED" or event == "UPDATE_SHAPESHIFT_FORM") then
	TSB_SpellBookFrame_Update(true)
    end
...
function TSB_SpellBookFrame_OnShow(self)
    ...
    -- NOTE: We'll be doing this OnShow every time the GUI opens AND when they switch between Spellbook/Pet tabs (which closes and re-opens the GUI).
      SpellBookFrame_Update(); -- NOTE: Instantly (synchronously) triggers "SPELLS_CHANGED" (and therefore our OnEvent) BEFORE returning back here.
      PlaySound(829);
...
function TSB_SpellBookFrame_OnHide(self)
    ...
    if (TSB_SpellBookFrame.bookType == BOOKTYPE_SPELL) then
        PlaySound(830); -- Normal book sound.
    else
        PlaySound(835); -- Pet book sound.
    end
    TSB_UpdateMicroButtons();
...
function TSB_SpellBookSkillLineTab_OnClick(self)
    ...
    if (TSB_SpellBookFrame:IsShown()) then
        PlaySound(856); -- Short, soft and gives nice feedback that a click happened. Perfect!
    end
end
...
function TSB_SkillLineFlash(action, ...)
    ...
    elseif (action == "hidecontainer") then
        ...
        UIFrameFlashStop(TSB_SpellBookTabFlashFrame); -- Stops flashing the container (but the container STAYS VISIBLE with 100% opacity).
    ...
    elseif (action == "synctoblizzard") then
        ...
        UIFrameFlashStop(SpellBookTabFlashFrame); -- Stops flashing the container (but the container STAYS VISIBLE with 100% opacity).
...
--        TSB_SpellBookFrameTabButton2:SetText(_G["PET_TYPE_"..petToken]); -- NOTE: Global strings: "PET_TYPE_PET = Pet, PET_TYPE_DEMON = Demon".
        TSB_SpellBookFrameTabButton2:Hide(); -- Pet tab.
...
function TSB_SpellButton_OnEnter(self)
    ...
    if (GameTooltip:SetSpellBookItem(spellId, TSB_SpellBookFrame.bookType)) then
...
function TSB_SpellButton_PostClick(self, button)
    ...
    if (IsModifiedClick("CHATLINK")) then
        if (MacroFrame and MacroFrame:IsShown()) then
            local spellName, subSpellName = GetSpellBookItemName(spellId, TSB_SpellBookFrame.bookType);
    ...
    if (IsModifiedClick("PICKUPACTION")) then
        PickupSpellBookItem(spellId, TSB_SpellBookFrame.bookType);
        return;
    end
end
...
-- Checks whether a spell-ID variable is legally usable in Blizzard's APIs such as "GetSpellBookItemName" (without them throwing errors).
...
-- or 0, or 1025, or nil, then their functions (such as "GetSpellBookItemName") will throw an error. If you try to look up a slot which simply doesn't
...
function TSB_RebuildSpellList()
    ...
    if (numSpells and numSpells >= 1) then -- Only scan if we have a non-nil, positive amount of spells in the given spell school/tab...
        ...
        for spellSlotID=(offset+1), SPELL_ID_TAB_MAX do -- The 1st slot on this tab is at "offset (which is all preceding tab's combined spell-counts) plus 1".
            ...
            currentSpellName = GetSpellBookItemName(spellSlotID, TSB_SpellBookFrame.bookType);
